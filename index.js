...
app.get('/charts', requireAuth, (req, res) => {
  ...
  const username = user.username;
  const historyRef = db.ref('history');
  // Get data from Firebase Realtime Database, sort by time and limit last 2 records
  historyRef.orderByChild('datetime').limitToLast(2).once('value', (snapshot) => {
    const data = snapshot.val();
    const historyArr = Object.values(data);
    const latestDistance = historyArr[1].average; // Get the most recent data
    const secondLatestDistance = historyArr[0].average; // Get the second closest data
    const warning = historyArr[1].warning;
    const difference =latestDistance - secondLatestDistance;
